...
class TableOfContentsGenerator():
    ...
    def generate_table_of_contents(self):
        ...
        if 'Bows' in self.weapon_types:
            parse_subnum += 1
            self.filter_file.write('# [{0}.{1}] - Bows\n'.format(str(parse_num).zfill(2), str(parse_subnum).zfill(2)))
        ...
        if 'Shields' in self.weapon_types:
            ...
            if 'A' in self.shield_types:
                parse_subsubnum += 1
                self.filter_file.write('# [{0}.{1}.{2}] - A Shields\n'.format(str(parse_num).zfill(2), str(parse_subnum).zfill(2), str(parse_subsubnum).zfill(2)))
